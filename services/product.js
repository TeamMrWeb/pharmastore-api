...
module.exports = {
    ...
    validateProduct: async (payload) => {
        try {
            const categoryId = await categoryService.findCategoryById(
                payload.categoryId
            )
            if (!categoryId) throw new Error('CategoryId doesnt exists')
            const inventoryId = await inventoryService.findInventoryById(
                payload.inventoryId
            )
            if (!inventoryId) throw new Error('InventoryId doesnt exists')
            const discountId = await discountService.findDiscountById(
                payload.discountId
            )
    ...
    getOne: async (id) => await Product.findByPk(id),
}
